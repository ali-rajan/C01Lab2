...
app.get('/getAllNotes', express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, 'secret-key', async (err, decoded) => {
      ...
      res.json({ response: await cursor.toArray() });
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete('/deleteNote/:noteId', express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, 'secret-key', async (err, decoded) => {
      ...
      const toDelete = await notesCollection.findOne(doc);
      if (!toDelete) {
        return res
          .status(404)
          .json({ error: 'Unable to find note with given ID.' });
      ...
      await notesCollection.deleteOne(doc); // TODO: is there a better way than to search the 2nd time for deleteOne?
...
app.patch('/editNote/:noteId', express.json(), async (req, res) => {
  try {
    ...
    const note = {};
    // TODO: find a better way to filter null fields this for an arbitrary number of fields
    ...
    if (!note.title && !note.content) {
      return res
        .status(400)
        .json({ error: 'At least one of title and content is required.' });
    }
    ...
    jwt.verify(token, 'secret-key', async (err, decoded) => {
      ...
      const toPatch = await notesCollection.findOne(doc);
      if (!toPatch) {
        return res
          .status(404)
          .json({ error: 'Unable to find note with given ID.' });
      ...
      // TODO: is there a better way than to use findOne then updateOne
      await notesCollection.updateOne(toPatch, {
        $set: note,
      });
